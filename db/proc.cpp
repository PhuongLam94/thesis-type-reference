...
void UserProc::constantPropagation(std::map<Exp*, ConstantVariable*>& map){
    ...
    while (worklist.size()>0){
        ...
        if (assign->isPhi()){
            ...
            val->type = 1;
            ConstantVariable* tempVal = NULL;
            for (int i=0; i<phiAssign->getNumDefs(); i++){
                Assignment* temp = (Assignment*) phiAssign->getStmtAt(i);
                std::map<Exp*, ConstantVariable*>::iterator mmit2;
                for (mmit2 = map.begin(); mmit2 != map.end(); mmit2++){
                    if ((*(*mmit2).first) == (*(new RefExp(temp->getLeft(), temp)))){
                        tempVal = (*mmit2).second;
                    }
                }
                if (tempVal){
        ...
        if (!val){
            ...
            std::cout<<"Value type: "<<val->type<<endl;
            if (val->variable)
                std::cout<<"Value: "<<val->variable->prints()<<endl;
                ...
                for (rit = listRTL->begin(); rit!=listRTL->end(); rit++){
                    ...
                    for (sit = stmts.begin(); sit!=stmts.end(); sit++){
                       ...
                       if (statement->isAssign()||statement->isPhi()){
                           ...
                           if (isDes){
                               if(std::find(worklist.begin(), worklist.end(), statement) == worklist.end()){
...
ProcSet* UserProc::decompile(ProcList* path, int& indent, std::map<Exp*, ConstantVariable*>&map, std::list<UnionDefine*>& unionDefine) {
		...
		for (PBB bb = cfg->getFirstBB(it); bb; bb = cfg->getNextBB(it)) {
			if (bb->getType() == CALL) {
				...
				if (c->status >= PROC_VISITED && c->status <= PROC_EARLYDONE) {
                    ...
                    ProcSet* tmp = c->decompile(path, indent, map, unionDefine);
	...
	if (child->size() == 0) {
		...
		earlyDecompile();
		std::cerr<<"AFTER initialise"<<std::endl;
        child = middleDecompile(path, indent, map, unionDefine);
	...
	if (child->size() == 0) {
		...
		if (*f == this) {
			...
			std::cout<<getTheReturnStatement()->getNumReturns()<<"\n";
            recursionGroupAnalysis(path, indent, map, unionDefine);// Includes remUnusedStmtEtc on all procs in cycleGrp
...
bool UserProc::unionCheck(std::list<UnionDefine*>& unionDefine, std::map<Exp*, ConstantVariable*> mapExp) {
...
//	mapLocalsAndParams();				// FIXME: unsure where this belongs
	...
	if (cfg->decodeIndirectJmp(this)) {
        ...
        ProcSet* ret = decompile(path, indent, map, unionDefine);	// Restart decompiling this proc
...
void UserProc::recursionGroupAnalysis(ProcList* path, int indent, std::map<Exp*, ConstantVariable*>&map, std::list<UnionDefine*>& unionDefine) {
	/* Overall algorithm:
	...
	*/
	...
	for (curp = cycleGrp->begin(); curp != cycleGrp->end(); ++curp) {
        (*curp)->middleDecompile(path, indent, map, unionDefine);
...
for (int i=0; i < 2; i++) {
	for (p = cycleGrp->begin(); p != cycleGrp->end(); ++p) {
        (*p)->remUnusedStmtEtc(map, unionDefine);				// Also does final parameters and arguments at present
	}
}
...
void UserProc::updateForUseChange(std::set<UserProc*>& removeRetSet, std::map<Exp*, ConstantVariable*>&map, std::list<UnionDefine*>& unionDefine) {
